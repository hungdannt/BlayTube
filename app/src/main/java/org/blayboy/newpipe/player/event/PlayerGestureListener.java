...
import static org.blayboy.newpipe.ktx.AnimationType.ALPHA;
import static org.blayboy.newpipe.ktx.AnimationType.SCALE_AND_ALPHA;
import static org.blayboy.newpipe.ktx.ViewUtils.animate;
import static org.blayboy.newpipe.player.Player.DEFAULT_CONTROLS_DURATION;
import static org.blayboy.newpipe.player.Player.DEFAULT_CONTROLS_HIDE_TIME;
import static org.blayboy.newpipe.player.Player.STATE_PLAYING;
...
public class PlayerGestureListener
    ...
    private void onScrollMainVolume(final float distanceX, final float distanceY) {
        ...
        if (player.getVolumeRelativeLayout().getVisibility() != View.VISIBLE) {
            animate(player.getVolumeRelativeLayout(), true, 200, AnimationType.SCALE_AND_ALPHA);
        }
    ...
    private void onScrollMainBrightness(final float distanceX, final float distanceY) {
        ...
        if (player.getBrightnessRelativeLayout().getVisibility() != View.VISIBLE) {
            animate(player.getBrightnessRelativeLayout(), true, 200, AnimationType.SCALE_AND_ALPHA);
        }
        if (player.getVolumeRelativeLayout().getVisibility() == View.VISIBLE) {
    ...
    public void onScrollEnd(@NotNull final MainPlayer.PlayerType playerType,
        ...
        if (playerType == MainPlayer.PlayerType.VIDEO) {
            ...
            if (player.getVolumeRelativeLayout().getVisibility() == View.VISIBLE) {
                animate(player.getVolumeRelativeLayout(), false, 200, AnimationType.SCALE_AND_ALPHA,
                        200);
            }
            if (player.getBrightnessRelativeLayout().getVisibility() == View.VISIBLE) {
                animate(player.getBrightnessRelativeLayout(), false, 200, AnimationType.SCALE_AND_ALPHA,
                        200);
            }

            if (player.isControlsVisible() && player.getCurrentState() == Player.STATE_PLAYING) {
                player.hideControls(Player.DEFAULT_CONTROLS_DURATION, Player.DEFAULT_CONTROLS_HIDE_TIME);
            }
        } else {
            if (player.isControlsVisible() && player.getCurrentState() == Player.STATE_PLAYING) {
                player.hideControls(Player.DEFAULT_CONTROLS_DURATION, Player.DEFAULT_CONTROLS_HIDE_TIME);
    ...
    public void onPopupResizingStart() {
        ...
        player.hideControls(0, 0);
        animate(player.getCurrentDisplaySeek(), false, 0, AnimationType.ALPHA, 0);
    }
